 ...
 * 
 * __Number 스펙__
 * - h1 : `font-weight: 700` `font-size: 24px` `line-height: 30px` `letter-spacing: 0px`
 * - h2 : `font-weight: 500` `font-size: 21px` `line-height: 26px` `letter-spacing: 0px`
 * - h3 : `font-weight: 500` `font-size: 19px` `line-height: 24px` `letter-spacing: 0px`
 * - body1 : `font-weight: 400` `font-size: 17px` `line-height: 22px` `letter-spacing: 0px`
 * - body1_M : `font-weight: 500` `font-size: 17px` `line-height: 22px` `letter-spacing: 0px`
 * - body2 : `font-weight: 400` `font-size: 16px` `line-height: 20px` `letter-spacing: 0px`
 * - body3 : `font-weight: 400` `font-size: 14px` `line-height: 18px` `letter-spacing: 0px`
 * - body3_M : `font-weight: 500` `font-size: 14px` `line-height: 18px` `letter-spacing: 0px`
 * - sub1 : `font-weight: 400` `font-size: 12px` `line-height: 16px` `letter-spacing: 0px`
 * - sub1_M : `font-weight: 500` `font-size: 12px` `line-height: 16px` `letter-spacing: 0px`
...
export default function Typo({ children, type, isNumber }: TypoProps) {
  const extractValue = typography[type];

  return (
    <TypoContainer font={extractValue} isNumber={isNumber}>
      <h2>{type}</h2> {children}
    </TypoContainer>
  );
}
...
const TypoContainer = styled.h1<{ font: string; isNumber?: boolean }>`
  ...
  ${({ isNumber }) => isNumber && `letter-spacing : 0`}
`;
